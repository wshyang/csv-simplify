...
### 2.1 General Rules
The program follows these general rules when simplifying and replacing the command strings:

- The environment, segment, intra_inter, and suffix_env must be consistent. For example, if the environment is production, the suffix_env must be prd. If the segment is intranet, the intra_inter must be intra. The suffix components must match the sensitive values for XXX, TLD, and YY. For example, if XXX is abc, TLD is com, and YY is sg, the suffix must be intraprd.abc.com.sg or interprd.abc.com.sg.
- The hostname must be converted to lowercase using casefold() before matching the regex and the specifications. This is to avoid case sensitive issues. For example, P2EAVWAABC01.INTRAPRD.ABC.COM.SG and p2eavwaabc01.intraprd.abc.com.sg are considered the same hostname.
...
### 2.2 How the References are Generated
...
### 2.3 Example Scenarios for PATH
...
`hostname_pattern = r'(?:(?:a-z0-9?\.)+(?:a-z0-9?)|(?:(?:a-z0-9?\.){2}(?:a-z0-9?))'`

This regex matches the following cases:

- A hostname that consists of one or more labels separated by dots, such as `p2eavwaabc01.intraPRD.abc.com.sg` or `localhost`
- A hostname that consists of two labels separated by dots, followed by another label, such as `p2eavwaabc01.intra.PRD` or `localhost.local.domain`

The regex captures the whole hostname in a group.

### 5.4 Hostname Format and Specifications
The program uses the following format and specifications for the hostnames:

- The hostname consists of four components: environment, segment, intra_inter, and suffix_env
- The environment is the first character of the hostname, and can be one of the following values: P (production), Q (quality), D (development), or T (testing)
- The segment is the second character of the hostname, and can be one of the following values: E (extranet), I (intranet), or A (application)
- The intra_inter is the third character of the hostname, and can be one of the following values: V (intranet), W (internet), or X (application)
- The suffix_env is the last part of the hostname, and consists of the sensitive values for XXX, TLD, and YY, separated by dots. For example, intraprd.abc.com.sg or interprd.abc.com.sg
- The hostname must follow the consistency rules for the environment, segment, intra_inter, and suffix_env, as described in the previous section
