...
- Replace numbers between 5 and 12 digits long that follow the word "echo" with the string "NUMERIC". For example, `echo 123456789` is replaced with `0` (where the original value `123456789` is in the references dataframe with an Index of 0).
- Replace valid hostnames with the string "HOSTNAME". A valid hostname follows the regex pattern defined in the global variable `hostname_pattern`. For example, `p2eavwaabc01.intraPRD.abc.com.sg` is replaced with `3` (where the original value `p2eavwaabc01.intraPRD.abc.com.sg` is in the references dataframe with an Index of 3).
